			...
			if (!is_wp_error($gitpage)) {
				if (isset($gitpage['body'])) {
					...
					if (false !== $pos) {
						...
						if ($semi > 0 ) {
							$pos += strlen($needle);
							$ver = substr($body, $pos, ($semi - $pos));

							$ct_ver = $ct->get('Version');

							// Set up admin notice on new version
							if (1 == strcmp($ver, $ct_ver)) {
								$up_start = $semi+2;
								$up_end = strpos($body,";;", $up_start);
								$whats_new = str_replace(';', ';<br/>', substr($body, $up_start, $up_end-$up_start+1));

								$this->admin_notices[] = array(
									'type'  => 'updated',
									'msg' => 'A new update of <strong>'. $ct->get('Name') .'</strong> is now available!<br/><br/>
											Your version:  <strong>' .  $ct_ver . '</strong><br/>
											New version:  <strong><span style="color: red;">' . $ver . '</span></strong><br/><br/>
											What\'s	new:<br/><pre>'. $whats_new .'</pre><br/>
											<a href="http://mas.creaws.com/">Get it now</a>&nbsp;&nbsp;|',
									'id'    => 'theme_notice_' . $ver,
									'dismiss'   => true,
								);
							}
						}
			...
			if ( empty($data) && !empty($type) ) {
				...
				**/
				if (!empty($type) && empty($data)) {
					...
					else if ($type == "elusive-icons" || $type == "font-icon" || $type == "font-icons" || $type == "icons" || $type == "fa-icons") {
						/**
		...
		function _admin_bar_menu()
		{
			...
			if($menu){
				...
				$nodeargs = array(
					...
					'title' => "<span class='ab-icon dashicons-admin-generic'></span>" . $theme_data->get('Name') . " " . __('Options', 'redux-framework'),
		...
		public function _enqueue_output() {
			...
			if ( ! empty( $this->typography ) && ! empty( $this->typography ) && filter_var( $this->args['output'], FILTER_VALIDATE_BOOLEAN ) ) {
				...
				if ( $this->args['async_typography'] && ! empty( $this->typography ) ) {
					...
					?>
					<style>.wf-loading *, .wf-inactive * {visibility: hidden;}
...
/*						wp_register_style( 'redux-google-fonts', $protocol . $typography->makeGoogleWebfontLink( $this->typography ), '', $version );
		...
		*/
		public function _enqueue() {
			...
			wp_register_style('redux-fa-icon',
				THEME_CSS_URI . '/font-awesome.css', array(),	0, 'all' );
...
*/
			...
			*/
			...
			$wp_styles->add_data( 'redux-css', 'rtl', true );
			wp_enqueue_style( 'select2-css' );
			//wp_enqueue_style( 'nouislider-css' );
			//wp_enqueue_style( 'qtip-css' );

			wp_enqueue_style( 'redux-fa-icon' );
...
*/
			...
			$arrEnq = array( 'jquery', 'select2-js' );
...
*/
								...
								if ( ( method_exists( $field_class, 'enqueue' ) ) || method_exists( $field_class, 'localize' ) ) {
									...
									if ( !wp_script_is( 'redux-field-'.$field['type'].'-js', 'enqueued' ) && class_exists($field_class) && method_exists( $field_class, 'enqueue' ) ) {
										...
										if ($this->args['dev_mode'] === false && isset($theField->extension_dir) && (!'' == $theField->extension_dir)  || ($this->args['dev_mode'] === true)) {
											...
											//$theField->enqueue(); // this is where all scripts and styles for fields are requested
										}
									}
			...
			wp_enqueue_script('fields-js'); // Enque the JS now
		...
		*/
		...
		public function get_header_html( $field ) {
			...
			if ( $this->args['default_show'] === true && isset( $field['default'] ) && isset($this->options[$field['id']]) &&
				$this->options[$field['id']] == $field['default'] && $field['type'] !== "info" && $field['type'] !== "section" && $field['type'] !== "editor" && $field['type'] !== "ace_editor" ) {
			...
			if( !empty( $plugin_options['import'] ) ) {
				...
				if ( !empty( $import ) ) {
					if (version_compare(phpversion(), "5.3.0", ">=")) {
						...
						$imported_options = $this->_array_cast_recursive($imported_options);
					}

					if (isset($imported_options['cws-' . THEME_SLUG])) {
						$cws_options = $imported_options['cws-' . THEME_SLUG];
						global $wpdb;
						foreach ($cws_options as $opt) {
							$res = $wpdb->query( "INSERT INTO $wpdb->options (option_name, option_value) VALUES('{$opt['option_name']}',
								'{$opt['option_value']}') ON DUPLICATE KEY UPDATE option_value = '{$opt['option_value']}'" );
						}
						unset($imported_options['cws-' . THEME_SLUG]);
					}
				}
		...
		*/
		public function _options_page_html() {
			...
			*/
			...
			$bgcolor = $_wp_admin_css_colors[$current_color]->colors[1];
			list($r, $g, $b) = sscanf($bgcolor, "#%02x%02x%02x");
			$bgstring = 'background:rgba('.$r.','.$g.','.$b.',.45);';
			echo '<style type="text/css" id="redux-custom-fonts-css">';
			echo '.redux-container>form{'.$bgstring.'}';
			echo '#redux-header-wrapper{'.$bgstring.'}';
			echo '#redux-footer{'.$bgstring.'}';
			echo '.redux-container #info_bar{'.$bgstring.'}';
			echo '</style>';
			echo '<input type="hidden" id="last_tab" name="' . $this->args['opt_name'] . '[last_tab]" value="' . $this->options['last_tab'] . '" />';

			// Header area
			echo '<div id="redux-header">';
			echo '<div id="redux-header-wrapper">';

			echo '<div id="redux-header-logo"><img src="' . self::$_url . 'assets/img/cws.png"/></div>';
			...
			if( !empty( $this->args['display_name'] ) ) {
				echo '<div class="display_header">';
					...
					echo "<p>";
					if (!empty($this->args['display_description']))
						echo $this->args['display_description'];
			...
			echo '<a href="javascript:void(0);" class="expand_options"></a>';

			// Warning bar
			echo '<div class="warning_bar">';
