...
$cws_shortcodes = array ('cws_progress', 'embed', 'alert', 'cws_button', 'quote', 'fa', 'featured_fa', 'dropcap', 'mark', 'ourteam', 'portfolio', 'cws_cta', 'news', 'tweets', 'table', 'milestone', 'services', 'shortcode_carousel', 'shortcode_blog');
...
function cws_shortcode_milestone ( $atts, $content = null ){
	...
	$out .= $fa ? "<div class='milestone_icon'><i class='icon_frame " . $fa . "'></i></div>" : "";
...
function shortcode_featured_fa($attr, $content = null) {
	...
	$out = "<i class='featured_icon ". $position ." $type " . $code . " fa-" . $size . ( $custom_color ? " custom_color" : "" ) . "'" . $color_styles . "></i>";
	return $out;
}
...
function shortcode_cws_cta($attr, $content = null) {
	...
	$icon5 = substr($icon, 0, 5);
	if ('fa fa' !== $icon5 && 'flati' !== $icon5) {
		$icon = 'fa fa-' . $icon;
	} else {
		$icon = 'fa ' . $icon;
	}

	$output .= $icon ? "<div class='icons_part icon_frame'><i class='" . $icon . "'></i></div>" : "";
	$output .= "<div class='content_wrapper'><div class='text_part'>" . ( $title ? "<div class='title'>$title</div>" : "" ) . do_shortcode($content) . "</div>" ;
	$output .= $link ? "<div class='button_part'><a class='cws_button medium' href='$link' " . ($target ? " target='_blank'" : "" ) . ">$button_text</a></div>" : "";
	$output .= '</div></div>';
...
function shortcode_services($attr) {
	...
	if (0 !== count($depts) ) {
		...
		foreach ($depts as $dept=>$v) {
			...
			$fa_check = sprintf('<i class="service_icon fa fa-2x %s"></i>',  !empty($fa_widget) ? $fa_widget : 'fa-check');
					...
					if (!empty($events)) {
						...
						if ( !isset($events->errors) ) {
							...
							if ($i) {
								...
								$homeurl = home_url();
								foreach ($events as $v) {
									$ev_links .= '<a href="' . $homeurl . '/?tribe_events_cat=' . $v->slug . '">' . $v->name . '</a>';
								}
								if (!empty($ev_links)) {
							$out .= '<div>';
...
function cws_shortcode_carousel ( $attr, $content){
	...
	$out .= "</div>";
	$out .= "<div class='carousel_content" . (!empty($autoplay) ? " autoplay" : "") . "' " . (!empty($autop_speed) ? " data-auto_p = '" . $autop_speed . "'" : "") . ">" . do_shortcode( $content ) . "</div>";
	$out .= "</div>";
	return $out;
}
