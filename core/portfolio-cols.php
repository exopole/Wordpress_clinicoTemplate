<?php
	if(!empty($_POST['action']) && is_array($_POST['action']) ) {
		$abspath = explode('/wp-content', dirname(__FILE__))[0];
...
function build_portfolio_item ($pageid, $cols, $is_carousel = false, $thumbnail_dims, $chars_count, $cp_type ) {
	...
	switch ($cp_type) {
		case 'portfolio':
			$out .= '<img src="' . $image . '" alt="' . get_post_meta(get_post_thumbnail_id(), "_wp_attachment_image_alt", true) . '">';
		...
		case 'staff':
			...
			if (!empty($cws_stored_meta)){
				$out .= '<img src="' . bfi_thumb($img[0], array('width' => 270, 'height' => 270) ) . '" alt="' . get_post_meta(get_post_thumbnail_id(), "_wp_attachment_image_alt", true) . '">';
				$out .= '<div class="hover-effect"></div>';
				$out .= '<div class="links">';
				if (count($cws_stored_meta)>0) {
					foreach ($cws_stored_meta as $social_item) {
						...
						$title_fa =  $social_item['cws-mb-socialgroup-fa'];

						$title_fa5 = substr($title_fa, 0, 5);
						if ('fa fa' !== $title_fa5 && 'flati' !== $title_fa5) {
							$title_fa = 'fa fa-' . $title_fa;
						}
						$out .= '<a ' . ( $url ? "href='$url' " : "" ) . ( $title ? "title='$title' " : "" ) . ( $title_fa ? "class='fa $title_fa' " : "" ) . ' target="_blank"></a>';
					}
				}
				$out .= "</div>";
			} else {
				$out .= '<a href='. $link .'><img src="' . bfi_thumb($img[0], array('width' => 270, 'height' => 270) ) . '" alt="' . get_post_meta(get_post_thumbnail_id(), "_wp_attachment_image_alt", true) . '"></a>';
			}
