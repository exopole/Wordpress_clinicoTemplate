					...
					<section class="news blog-post staff">
						<div class="item">
							...
							while ( have_posts() ): the_post();
								...
								$social_meta = isset($cws_stored_meta['social']) ? $cws_stored_meta['social'] : null;
								...
								for ($i=0;$i<count($social_meta);$i++){
									if ( (!empty($social_meta[$i]['cws-mb-socialgroup-fa'])) && (!empty($social_meta[$i]['cws-mb-socialgroup-url'])) ) {
										$fa = $social_meta[$i]['cws-mb-socialgroup-fa'];
										$fa5 = substr($fa, 0, 5);
								...
								$staff_info = array(
									__('Disponible au ', THEME_SLUG ) => wp_get_post_terms( get_the_ID(), 'cws-staff-dept'),
									...
									__('Occupation', THEME_SLUG ) => isset( $cws_stored_meta['cws-staff-degree'] ) ? esc_attr( $cws_stored_meta['cws-staff-degree'] ) : '',
